...
def init_db(*, ensure_schema: bool = False) -> None:
    """
    Verify connectivity.

    Schema changes are applied explicitly with `alembic upgrade head` (not on Uvicorn startup).
    `ensure_schema=True` is for tests/local helpers only (create_all) — never used in production startup.
    """
    ...
    if ensure_schema:
        ...
        logger.info("cloud database schema ensured via create_all (non-production helper)")


def run_alembic_upgrade() -> None:
    """Apply migrations via Alembic. Call from deploy scripts / CLI, not Uvicorn startup."""
    _run_alembic_upgrade()
    logger.info(
        "cloud database migrations applied (%s)",
